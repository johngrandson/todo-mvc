    ...
    pub fn active_filters(&self) -> usize {
        ...
        for entry in &self.entries {
            if self.filter.fits(&entry) {
                if self.active_filter.is_active(entry) {
                    counter = counter + 1;
                }
            }
        }
...
impl ActiveFilter {
    pub fn is_active(&self, entry: &Entry) -> bool {
        match *self {
            ActiveFilter::Active => !entry.completed,
            ActiveFilter::Completed => entry.completed,
            ActiveFilter::Even => entry.idx % 2 == 1,
            ActiveFilter::Odds => entry.idx % 2 == 0,
        }
    }
}
...
impl Filter {
    pub fn fits(&self, entry: &Entry) -> bool {
        match *self {
            ...
            Filter::Even => entry.idx % 2 == 1,
            Filter::Odds => entry.idx % 2 == 0,
        }
    }
